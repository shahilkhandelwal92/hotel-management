...
export async function PUT(request: NextRequest, { params }: { params: Params }) {
    ...
    try {
        ...
        const { name, corporateName, date, expectedCount, status } = body;
        const event = await prisma.corporateEvent.update({
            where: { id },
            data: {
                ...(name && { name: name.trim() }),
                ...(corporateName && { corporateName: corporateName.trim() }),
                ...(date && { date: new Date(date) }),
                ...(expectedCount !== undefined && { expectedCount: parseInt(String(expectedCount), 10) }),
                ...(status && { status }),
            },
        });
...
export async function DELETE(req: NextRequest, { params }: { params: Params }) {
    ...
    try {
        ...
        const guestIds = existing.guests.map((g) => g.id);

        await prisma.$transaction(async (tx) => {
            if (guestIds.length > 0) {
                await tx.guestRequest.deleteMany({ where: { guestId: { in: guestIds } } });
                await tx.corporateGuest.deleteMany({ where: { eventId: id } });
            }
        ...
        await logAudit({
            ...
            action: "DELETE",
            entityId: id,
            oldValue: { name: existing.name, corporateName: existing.corporateName },
            req,
        });
